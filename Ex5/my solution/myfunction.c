...
// 				maxGreen = imageCopy[prevRow + prevGreen];
// 				maxBlue = imageCopy[prevRow + prevBlue];
// 			} else if (pixel11 <= minPixelIntensity) {
// 				maxRed = imageCopy[prevRow + prevRed];
// 				maxGreen = imageCopy[prevRow + prevGreen];
// 				maxBlue = imageCopy[prevRow + prevBlue];
// 			}
...
void sharpen(Image *image) {
	...
	for (row = 1; row < m-1; ++row) { // the number of rows is m and not n because we want span the row acccorss one dimension
		...
		for (column = 3; column < sizeOfRow - 3; column += 3) { //column is the index of the pixel in the center of the square
			...
			// The Reds Value
			//first row
			borderRedsValue = imageCopy[prevRow + prevRed] + imageCopy[prevRow + column] + imageCopy[prevRow + nextRed] + 
			imageCopy[currRow + prevRed] + imageCopy[currRow + nextRed] + imageCopy[nextRow + prevRed] +
			imageCopy[nextRow + column] + imageCopy[nextRow + nextRed];
			...
			centerRedValue = imageCopy[currRow + column] + (imageCopy[currRow + column] << 3);
			centerGreenValue = imageCopy[currRow + currGreen] + (imageCopy[currRow + currGreen] << 3);
			centerBlueValue = imageCopy[currRow + currBlue] + (imageCopy[currRow + currBlue] << 3);
